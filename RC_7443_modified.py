...
from velocity_functions import nloglike_Bur, v_co_Burket, v_co_Burket_nb,v_d, vel_h_Burket, vel_b, nloglike_Bur_nb
...
for i in range(len(DTable_list)):
    ...
    if list(r)[int(len(r)/2)] != max(list(r)[:int(len(r)/2)]):
            ...
            plt.plot(r_normalized, vel_h_Burket(np.array(r) * 1000, bestfit_av.x[4], bestfit_av.x[5] * 1000),
                 label='Burket halo')
            plt.legend()
            ...
            bestfit_mav = minimize(nloglike_Bur, p0, args=(r, mav, mav_err),
                               bounds=param_bounds)
            print('---------------------------------------------------')
            ...
            # -------------------------------------------------------------------------------
            bestfit_miv = minimize(nloglike_Bur, p0, args=(r, np.abs(miv), miv_err),
                               bounds=param_bounds)
            print('---------------------------------------------------')
            ...
            plt.plot(r_normalized, vel_h_Burket(np.array(r) * 1000, bestfit_av.x[2], bestfit_av.x[3] * 1000),
                    label='Burket halo')
            plt.legend()
            plt.xlabel('$r_{dep}$/$R_d$')
            plt.ylabel('$v_{rot}$ [km/s]')
            ...
            plt.plot(r_normalized_mav, v_d(np.array(r) * 1000, 10 ** (bestfit_mav.x[0]), bestfit_mav.x[1] * 1000),
                 label='disk')
            plt.plot(r_normalized_mav, vel_h_Burket(np.array(r) * 1000, bestfit_mav.x[2], bestfit_mav.x[3] * 1000),
                 label='Burket halo')
            plt.legend()
            plt.xlabel('$r_{dep}$/$R_d$')
            plt.ylabel('$v_{rot}$ [km/s]')
            ...
            plt.plot(r_normalized_miv, v_d(np.array(r) * 1000, 10 ** bestfit_miv.x[0], bestfit_miv.x[1] * 1000),
                 label='disk')
            plt.plot(r_normalized_miv, vel_h_Burket(np.array(r) * 1000, bestfit_miv.x[2], bestfit_miv.x[3] * 1000),
                 label='Burket halo')
            plt.legend()
            plt.xlabel('$r_{dep}$/$R_d$')
            plt.ylabel('$v_{rot}$ [km/s]')
