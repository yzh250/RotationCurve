...
for i in range(len(rho_dc)):
    ...
    print('---------------------------------------------------')
    print(bestfit_av)
    bestfit_list.append(bestfit_av.x)
...
'''
        ...
        good_fit_flag = True
        if (bestfit_av.x[2] == rho_dc[i]) or (bestfit_av.x[2] == rho_min) or (bestfit_av.x[2] == rho_max):
            good_fit_flag = False
        if (bestfit_av.x[3] == r_h[j]) or (bestfit_av.x[3] == rhalo_min) or (bestfit_av.x[3] == rhalo_max):
            good_fit_flag = False
        if good_fit_flag:
...
Good_fit_Table = QTable([rho_good, rho_good_initial, rhalo_good, rhalo_good_initial], names=('Rho_dc (M_sol/pc^3)','Rho_dc_initial (M_sol/pc^3)', 'R_halo (kpc)', 'R_halo_initial (kpc)'))
print(Good_fit_Table)

ascii.write([rho_good, rho_good_initial, rhalo_good, rhalo_good_initial], 'good_fit_re.txt', format='ecsv', names=('Rho_dc (M_sol/pc^3)','Rho_dc_initial (M_sol/pc^3)', 'R_halo (kpc)', 'R_halo_initial (kpc)'), overwrite=True)
'''