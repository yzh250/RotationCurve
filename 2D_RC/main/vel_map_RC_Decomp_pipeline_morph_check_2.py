...
for i in range(len(galaxy_ID)):
    ...
    if path.exists(data_file):
        ...
        elif map_smoothness > max_map_smoothness:
            print(galaxy_ID[i] + ' a smoothness score greater than 2')
            if Ttype > 0:
                ttype_g_0 += 1
        ...
        '''
        if (map_smoothness <= max_map_smoothness) and (Ttype > 0):
            gal_pass += 1
            print(galaxy_ID[i] + ' passed the initial two cuts')
            if tidal == 0:
                gal_ntidal += 1
                print(galaxy_ID[i] + ' has no tidal presence')
                if incl > 0:
                    gal_pincl += 1
                    print(galaxy_ID[i] + ' has valid inclination')
                    if unmasked_data == True:
                        gal_unmask += 1
                        print(galaxy_ID[i] + ' has most of data unmasked')
                    else:
                        gal_mask += 1
                        print(galaxy_ID[i] + ' has most of data masked')
                else:
                    gal_nincl += 1
                    print(galaxy_ID[i] + ' has no valid inclination')
            else:
                gal_tidal += 1
                print(galaxy_ID[i] + ' has tidal presence')
        else:
            gal_npass += 1
            print(galaxy_ID[i] + ' did not passe the initial two cuts')
    else:
...
print(str(gal_pass) + ' galaxies pass the two initial cuts (smoothness <= 2, Ttype > 0)')
print(str(gal_npass) + ' galaxies did not pass the two initial cuts')
print(str(gal_ntidal) + ' galaxies have no tidal presence')
print(str(gal_tidal) + ' galaxies have tidal presence')
print(str(gal_pincl) + ' galaxies have valid inclination')
print(str(gal_nincl) + ' galaxies have no valid inclination')
print(str(gal_unmask) + ' galaxies have most of data unmasked')
print(str(gal_mask) + ' galaxies have most of data masked')
