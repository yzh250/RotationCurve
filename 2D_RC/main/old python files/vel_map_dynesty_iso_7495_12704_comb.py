...
dsampler = dynesty.DynamicNestedSampler(loglikelihood_iso_flat_constraints, prior_isothermal, 
												 logl_args = (scale,
												 	          gshape,
												 	          data_maps['vmasked'].compressed(),
												 	          data_maps['ivar_masked'].compressed(),
...
f_output = open('dynesty_results_' + gal_ID + '_comb.pickle','wb')
pickle.dump(d_result,f_output)
f_output.close()

#np.save('dynesty_results_' + gal_ID + '_comb.npy', d_result)
