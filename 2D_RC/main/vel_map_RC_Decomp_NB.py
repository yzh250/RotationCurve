...
from RC_2D_Fit_Functions import Galaxy_Data, \
                                Galaxy_Fitting_iso_nb,\
                                Galaxy_Fitting_NFW_nb, \
                                Galaxy_Fitting_bur_nb, \
                                Hessian_Calculation_Isothermal,\
                                Hessian_Calculation_NFW,\
                                Hessian_Calculation_Burket,\
                                Plotting_Isothermal_nb,\
                                Plotting_NFW_nb,\
                                Plotting_Burket_nb,\
                                getTidal
...
# Isothermal
c_iso = open('iso nb.csv','w')
writer_iso = csv.writer(c_iso)
writer_iso.writerow(['galaxy_ID', 'SigD', 'Rd', 'rho0_h', 'Rh', 'incl', 'phi', 'x_cen', 'y_cen','Vsys','chi2'])

# NFW
c_nfw = open('nfw nb.csv','w')
writer_nfw = csv.writer(c_nfw)
writer_nfw.writerow(['galaxy_ID', 'SigD', 'Rd', 'rho0_h', 'Rh', 'incl', 'phi', 'x_cen', 'y_cen','Vsys','chi2'])

# Burket
c_bur = open('bur nb.csv','w')
writer_bur = csv.writer(c_bur)
writer_bur.writerow(['galaxy_ID', 'SigD', 'Rd', 'rho0_h', 'Rh', 'incl', 'phi', 'x_cen', 'y_cen','Vsys','chi2'])
...
for i in range(len(galaxy_ID)):
    ...
    if path.exists(data_file):
            ...
            if map_smoothness <= max_map_smoothness and tidal == 0:
                ...
                Isothermal_fit = Galaxy_Fitting_iso_nb(parameters,
                                                    scale,
                                                    gshape,
                                                    vmasked,
                                                    Ha_vel_ivar)

                NFW_fit = Galaxy_Fitting_NFW_nb(parameters,
                                             scale,
                                             gshape,
                                             vmasked,
                                             Ha_vel_ivar)

                Burket_fit = Galaxy_Fitting_bur_nb(parameters,
                                                scale,
                                                gshape,
                                                vmasked,
                                                Ha_vel_ivar)
                ...
                # Plotting
                # -------------------------------------------------------------------------------
                Plotting_Isothermal_nb(galaxy_ID[i], gshape, scale, Isothermal_fit, Ha_vel_mask)
                Plotting_NFW_nb(galaxy_ID[i], gshape, scale, NFW_fit, Ha_vel_mask)
                Plotting_Burket_nb(galaxy_ID[i], gshape, scale, Burket_fit, Ha_vel_mask)
                # -------------------------------------------------------------------------------
                ...
                full_vmap_iso = rot_incl_iso_nb(gshape, scale, Isothermal_fit)
                ...
                full_vmap_NFW = rot_incl_NFW_nb(gshape, scale, NFW_fit)
                ...
                full_vmap_bur = rot_incl_bur_nb(gshape, scale, Burket_fit)
                ...
                # -------------------------------------------------------------------------------
                writer_iso.writerow([galaxy_ID[i],Isothermal_fit[0],Isothermal_fit[1],Isothermal_fit[2],Isothermal_fit[3],Isothermal_fit[4],Isothermal_fit[5],Isothermal_fit[6],Isothermal_fit[7],Isothermal_fit[8],chi2_iso_norm])
                writer_nfw.writerow([galaxy_ID[i],NFW_fit[0],NFW_fit[1],NFW_fit[2],NFW_fit[3],NFW_fit[4],NFW_fit[5],NFW_fit[6],NFW_fit[7],NFW_fit[8],chi2_NFW_norm])
                writer_bur.writerow([galaxy_ID[i],Burket_fit[0],Burket_fit[1],Burket_fit[2],Burket_fit[3],Burket_fit[4],Burket_fit[5],Burket_fit[6],Burket_fit[7],Burket_fit[8],chi2_bur_norm])
                # -------------------------------------------------------------------------------
                ...
                writer_iso.writerow([galaxy_ID[i], 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A','N/A'])
                writer_nfw.writerow([galaxy_ID[i], 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A','N/A'])
                writer_bur.writerow([galaxy_ID[i], 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A','N/A'])
    else:
